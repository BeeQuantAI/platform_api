...
export class PasswordValidationPipe implements PipeTransform {
  ...
  transform(value: any) {    
    ...
    if (error) {
      ...
      if (
        error.message.match(/^"Old Password" with value.*fails to match the required pattern: .*/) ||
        error.message.match(/^"New Password" with value.*fails to match the required pattern: .*/)
      ) {
        throw new InvalidInputException("Validation failed: password must contain 8 to 32 characters, " +
