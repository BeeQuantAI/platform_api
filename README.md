...
OAUTH_REDIRECT_URI=http://localhost:3010/login
```
...
6. Wait for the Container to Start and check the container's status:

   ```
   docker ps
   ```

7. Download a DBeaver: https://dbeaver.io/download/, connect your db using DBeaver by using the following config:

8. Download a DBeaver: https://dbeaver.io/download/, connect your db using DBeaver by using the following config:
...
9. (optional) Stop and remove the container: (you need to run step 3 again once you remove it):
10. (optional) Stop and remove the container: (you need to run step 3 again once you remove it):
    ```
    docker stop bqCore
    docker rm bqCore
    ```
...
1. Navigate to `http://localhost:3000/graphql` in your web browser to test queries and mutations using the interactive GraphQL playground.
